...
final class MenuBarController: NSObject {
    ...
    private func openPanel() {
        ...
        // Popover open refreshes Codex, Claude, and CommandCode (all through
        // prompt-free read-only paths). Each coordinator leg has a 60s
        // cooldown so rapid open/close doesn't repeat work.
        Task { await appState.refreshCodexRateLimitIfNeeded() }
        Task { await appState.refreshClaudeRateLimitIfNeeded() }
        Task { await appState.refreshCommandCodeRateLimitIfNeeded() }
